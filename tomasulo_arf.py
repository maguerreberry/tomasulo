...
class ARFobject:
    ...
    def reg_print(self):
        print ("------------------------------------------------------------------------------------")
        print ("REGISTROS ENTEROS DISTINTOS DE CERO")
        print ("------------------------------------------------------------------------------------")    
        row_format ="{:^10}" * 2
        row_format_value ="{:^10.3f}" * 16
        
        print (row_format.format(*["NOMBRE", "VALOR"]))
        for address, word in enumerate(self.int_registers):
            if word != 0:
                print (row_format.format(*["R" + str(address), word]))
        ...
        print
        print ("------------------------------------------------------------------------------------")
        print ("REGISTROS DE PUNTO FLOTANTE DISTINTOS DE CERO")
        print ("------------------------------------------------------------------------------------")    
        print (row_format.format(*["NOMBRE", "VALOR"]))
        for address, word in enumerate(self.fp_registers):
            if word != 0:
                print (row_format.format(*["F" + str(address), word]))


        # reg_names = ["F0", "F1", "F2", "F3", "F4", "F5", "F6", "F7", "F8", "F9", "F10", "F11", "F12", "F13", "F14", "F15"]
        # print (row_format.format(*reg_names))
        # print (row_format_value.format(*self.fp_registers[0:16]))
        # reg_names = ["F16", "F17", "F18", "F19", "F20", "F21", "F22", "F23", "F24", "F25", "F26", "F27", "F28", "F29", "F30", "F31"]
        # print (row_format.format(*reg_names))
        # print (row_format_value.format(*self.fp_registers[16:32]))
        print